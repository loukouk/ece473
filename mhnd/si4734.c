...
//send fm power up command
  ...
  si4734_wr_buf[1] = 0x50 | 0x80;      //GPO2O enabled, STCINT enabled, use ext. 32khz osc.
...
//command is guarnteed by design to finish in 10ms. 
//
void get_property(uint16_t property){
    si4734_wr_buf[0] = GET_PROPERTY;              
    si4734_wr_buf[1] = 0x00;                           //all zeros
    si4734_wr_buf[2] = (uint8_t)(property >> 8);       //property high byte
    si4734_wr_buf[3] = (uint8_t)(property);            //property low byte

    twi_start_wr(SI4734_ADDRESS, si4734_wr_buf, 4); //send get_int_status command
    while( twi_busy() ){}; //spin while previous TWI transaction finshes
    _delay_ms(300);  //SET_PROPERTY command takes 10ms to complete
    twi_start_rd(SI4734_ADDRESS, si4734_rd_buf, 4); //get the interrupt status 
    while( twi_busy() ){}; //spin while previous TWI transaction finshes
    _delay_ms(10);        //si4734 process delay
    PORTB = si4734_rd_buf[3];
